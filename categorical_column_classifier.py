...
import hdbscan
import pandas as pd
from pycaret.classification import setup, create_model, tune_model, finalize_model, predict_model, save_model, plot_model
...
def main(column_name='ExteriorColour'):
    ...
    cluster_labels = data.groupby('cluster')[column_name].agg(lambda x: x.value_counts().index[0])
    data['cluster'] = data['cluster'].map(cluster_labels)
    training_data = pd.concat([data['cluster'].reset_index(), pd.DataFrame(embeds)], axis=1)
    training_data = training_data.drop('index', axis=1)
    classifier = setup(training_data, target='cluster')
    rf = create_model('rf')
    tuned_rf = tune_model(rf)
    final_rf = finalize_model(tuned_rf)
    predictions = predict_model(final_rf, data=training_data)
    if column_name == 'ExteriorColour':
        save_model(final_rf, 'colour_classifier_model')
    elif column_name == 'StereoDescription':
        save_model(final_rf, 'stereo_classifier_model')
