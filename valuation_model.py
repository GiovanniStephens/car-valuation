...
def classify_colour(data, embedding_model):
    ...
    model = load_model("models/colour_classifier_model")
    prediction_data = pd.concat(
        [data["ExteriorColour"].reset_index(), pd.DataFrame(embeds)], axis=1
    )
...
def classify_stereo(data, embedding_model):
    ...
    model = load_model("models/stereo_classifier_model")
    prediction_data = pd.concat(
        [data["StereoDescription"].reset_index(), pd.DataFrame(embeds)], axis=1
    )
    prediction_data = prediction_data.drop("index", axis=1)
    predictions = predict_model(model, data=prediction_data)
...
def identify_undervalued_cars(data, config, listingIds, model):
    ...
    undervalued = predictions[
        (
            predictions["price"]
            < predictions["prediction_label"] - undervalued_filter_min
        )
        & (
            predictions["price"]
            > predictions["prediction_label"] - undervalued_filter_max
        )
    ]
    ...
    for filter_rule in filters:
        ...
        value = filter_rule["value"]  # noqa: F841 - used in query via @value
...
def main():
    ...
    save_model(final_rf, f"models/{make}_{model}_final_rf_model")
    _ = predict_model(final_rf, data=out_of_sample)
...
def value_input_car():
    ...
    model = load_model(f"models/{car_make}_{car_model}_final_rf_model")
