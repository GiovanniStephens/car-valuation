...
def find_undervalued(
    make, model, min_discount_pct, max_discount_pct, filters, comparison_region=None
):
    """Find undervalued cars for a single make/model."""
    from autogluon.tabular import TabularPredictor
    from utils import load_config, load_data, get_model_path
    from feature_engineering import feature_engineering
    ...
    try:
        ...
        data["discount_pct"] = (
            (data["prediction_label"] - data["price"]) / data["prediction_label"] * 100
        )
...
def main():
    ...
    parser.add_argument(
        "--train-general",
        action="store_true",
        help="Also train general cross-car model",
    )
    ...
    display_df["prediction_label"] = display_df["prediction_label"].apply(
        lambda x: f"${x:,.0f}"
    )
    display_df["discount_pct"] = display_df["discount_pct"].apply(lambda x: f"{x:.1f}%")
    if "comparison_value" in display_df.columns:
        display_df["comparison_value"] = display_df["comparison_value"].apply(
            lambda x: f"${x:,.0f}"
        )
        display_df["regional_diff_pct"] = display_df["regional_diff_pct"].apply(
            lambda x: f"{x:+.1f}%"
        )
    col_renames = {
        "prediction_label": "Est. Value",
        "discount_pct": "Discount",
        "comparison_value": f"{comparison_region} Value"
        if comparison_region
        else "Comp. Value",
